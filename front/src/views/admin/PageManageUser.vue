          ...
          <tbody>
            <!-- v-for로 반복 돌려서 데이터 가져와서 링크 넣고 뿌려주기 -->
              <tr>
                ...
                <td rowspan="2">(user.profile_img)</td>
                <td rowspan="2">(user.email)</td>
                <td rowspan="2">(user.name)</td>
                <td rowspan="2">(user.age)</td>
                <td rowspan="2">(user.gender)</td>
                <td rowspan="2">(user.nickname)</td>
                <td rowspan="2">(user.phone)</td>
                <td class="flex">
                  <!-- 비활성화 / 활성화 버튼, 회원의 suspend_until 컬럼으로 상태 파악 -->
                  <button v-if="trigger" class="btn my-red-button"  data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@mdo" @click="changeTrigger(1)"><span>비</span></button>
                  <button v-else class="btn my-blue-button" @click="changeTrigger(1)"><span>활</span></button>
                  <!-- 탈퇴 버튼 -->
