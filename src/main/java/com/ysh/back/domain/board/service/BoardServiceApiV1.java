...
public class BoardServiceApiV1 {
    ...
    public ResponseEntity<?> getBoardListData(String search, String sort, Boolean desc) {
        ...
        if ((search != null) && (sort != null)) {
            ...
            // 검색어만 입력시
            boardEntityList = boardRepository
                    .findByNameContainingOrContentContainingOrUserEntity_NameContainingAndDeletedAtIsNull(search,
                            search, search);
        } else if (sort != null) {
            // 정렬 기준만 입력 시
            if (desc.equals(true)) {
    ...
    public ResponseEntity<?> getBoardCommentListData(Long boardIdx) {
        List<CommentEntity> commentEntityList = commentRepository
                .findByBoardEntityIdxAndDeletedAtIsNullOrderByCreatedAtDesc(boardIdx);
    ...
    @Transactional
    public ResponseEntity<?> deleteBoardData(Long boardIdx, CustomUserDetails customUserDetails) {
        Optional<BoardEntity> boardEntityOptional = boardRepository.findById(boardIdx);
        ...
        if (boardEntity.getUserEntity().getIdx() != userRepository.findByEmail(customUserDetails.getUsername()).get()
                .getIdx()) {
            throw new BadRequestException("해당 게시물을 삭제할 권한이 없습니다.");
        }

        String oldVal = "제목 : " + boardEntity.getName()
                + "내용 : " + boardEntity.getContent();

        boardEntity.setDeletedAt(LocalDateTime.now());
        boardEntity.setName("삭제된 게시물입니다.");
        boardEntity.setContent("삭제된 게시물입니다.");
    ...
    public ResponseEntity<?> insertBoardReportData(Long boardIdx, ReqBoardReportDTO reqBoardReportDTO,
        ...
        Optional<BoardReportEntity> boardReportEntityOptional = boardReportRepository.findByBoardIdxAndUserIdx(boardIdx,
                userRepository.findByEmail(customUserDetails.getUsername()).get().getIdx());
    ...
    public ResponseEntity<?> insertBoardRecommendData(Long boardIdx, CustomUserDetails customUserDetails) {
        Optional<BoardRecommendEntity> boardRecommendEntityOptional = boardRecommendRepository.findByBoardIdxAndUserIdx(
                boardIdx, userRepository.findByEmail(customUserDetails.getUsername()).get().getIdx());
